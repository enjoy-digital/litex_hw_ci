...
litex_ci_configs = {
    ...
    "acorn_baseboard_mini_naxriscv_64_bit" : LiteXCIConfig(
        ...
        gateware_command = f"--sys-clk-freq 156.25e6 \
        --cpu-type=naxriscv --xlen {fixme} --scala-args='rvc=true,rvf=true,rvd=true' --with-rvc --with-fpu \
        --with-coherent-dma \
        --with-ethernet --eth-ip={local_ip} --remote-ip={remote_ip}",
        software_command = f"cd linux && python3 make.py --cpu-type=naxriscv --xlen {fixme} --soc-json=../build_acorn_baseboard_mini_naxriscv_64_bit/soc.json --linux-clean --linux-build --linux-generate-dtb --linux-prepare-tftp",
        setup_command    = "",
        exit_command     = "",
        tty              = "/dev/ttyUSB1",
